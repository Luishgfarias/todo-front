...
const CardTask = (task: Task) => {
    const { taskByID, deleteTask, selectTask, selectedTasks, unselectTask, toggleTask, loadingTaskId } = useTaskStore();
    ...
    const isAnyCardLoading = loadingTaskId !== null;
    ...
    return (
        <div className={`flex justify-between items-center w-full p-3 bg-white border-2 rounded-lg transition-all ${borderColor()} ${isDisabled ? 'opacity-50 cursor-not-allowed' : 'hover:shadow-md cursor-pointer'}`}>
            ...
            {isThisCardLoading ? (
                <div className="flex items-center gap-3 flex-1 w-full justify-center">
                    <Loading size="sm" message="Carregando detalhes..." />
                </div>
            ) : (
                <button 
            ...
            <div className="flex gap-2 items-center">
                <button
                    onClick={handleDone}
                    className="p-2 text-green-500 hover:text-green-700 hover:bg-green-50 rounded-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    disabled={isDisabled}
                >
